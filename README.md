...
graph TB
    subgraph UI["🖥 用户界面层"]
        WEB["Web 前端<br/>Bootstrap 5 + vanilla JS"]
    end
    ...
    subgraph ORCH["⚙ 编排层"]
        WF["InkAIWorkflowOptimized<br/>140KB 核心编排器"]
        QCE["QuickContinuationExecutor<br/>异步续写引擎"]
    end

    subgraph AGENTS["🤖 智能体层 (25 Agents)"]
        CREATION["创作 Agent ×5<br/>标签 → 角色 → 故事线 → 写作 → 评估"]
        CONT["续写 Agent ×3<br/>知识库 → 续写线 → 续写章"]
        ASSESS["评估 Agent ×6<br/>角色/情节/世界观/风格/体验/长线"]
        IMPROVE["改进 Agent ×11<br/>按维度专项修复"]
    end
...
sequenceDiagram
    participant U as 👤 用户
    participant W as 编排器
    participant TS as 标签选择
    participant CC as 角色创建
    participant SG as 故事线
    participant CW as 章节写作
    participant QA as 质量评估

    U->>W: 输入需求（书名 + 类型 + 一句话描述）
    W->>TS: 分析需求 → 推荐标签
    TS-->>W: 四维标签（类型/主题/风格/受众）
    W->>CC: 标签 + 需求 → 创建角色
    CC-->>W: 主角档案 + 配角群
    W->>SG: 角色 + 标签 → 构建故事
    SG-->>W: 三幕剧 × 章纲 × 伏笔
